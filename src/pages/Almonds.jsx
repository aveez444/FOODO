...
// LOCAL IMAGE IMPORTS
import demo_1 from '../assets/products/almonds.jpg';
import demo_2 from '../assets/products/Demos/Almond Back IMG.jpeg';
import demo_3 from '../assets/products/Demos/almonds_.png';
import almondVideo from '../assets/products/Demos/Almond_Demo.mp4';

import demo2 from '../assets/products/mixed-nuts.jpeg';
...
const productData = {
  ...
  currency: '₹',
  inStock: false,
  short: 'Premium quality almonds, rich in nutrients and perfect for snacking or cooking.',
  images: [demo_1, demo_2, demo_3, almondVideo],
};

const related = [
  {
    id: 'rel-1',
    title: 'Food-O Mixed Nuts (200g)',
    price: 250,
    compareAt: 160,
    discount: 28,
    rating: 4.6,
...
function DriveRates({ onPriceChange }) {
  const [weight, setWeight] = useState(200);
  const weightOptions = [
    { weight: 200, price: 160.00, label: 'Small Pack' },
    { weight: 500, price: 400.00, label: 'Medium Pack' },
    { weight: 1000, price: 800.00, label: 'Large Pack' },
    { weight: 2000, price: 1500.99, label: 'Family Pack' },
  ];

  useEffect(() => {
    const selected = weightOptions.find((option) => option.weight === weight);
    if (selected) {
      onPriceChange(selected.price);
    }
  }, [weight, onPriceChange]);
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border p-4">
      ...
      <div className="mt-3 grid gap-3">
          ...
          {weightOptions.map((option) => (
            ...
            >
              ...
              <div className="text-xs">
                {option.weight >= 1000 ? `${option.weight / 1000}kg` : `${option.weight}g`} • ₹{option.price.toFixed(2)}
              </div>
            </button>
          ))}
        </div>
        <div className="mt-2 flex items-center justify-between rounded-lg bg-neutral-50 p-3">
          ...
          <div className="text-lg font-bold">₹{(weightOptions.find((option) => option.weight === weight)?.price || 160).toFixed(2)}</div>
        </div>
      </div>
    </motion.div>
  );
...
export default function ProductDetailPage() {
  ...
  const price = useMemo(() => (currentPrice * qty).toFixed(2), [qty, currentPrice]);

  const handleAddToCart = () => {
    addToCart({ ...productData, price: currentPrice }, qty);
    setToast(`Added ${qty} ${productData.title} to cart — ₹${price}`);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-4">
            <span className="text-lg font-extrabold">Premium Nuts</span>
          </div>
        ...
        <div className="mt-4 grid gap-6 md:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-4">
              ...
              <div className="text-3xl font-bold tracking-tight">₹{currentPrice.toFixed(2)}</div>
              <p className="text-sm leading-relaxed text-neutral-700">{productData.short}</p>
              <DriveRates onPriceChange={setCurrentPrice} />
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-blue-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-600 md:flex-none md:px-6"
                >
      ...
      <BuyNowModal isOpen={isBuyNowOpen} onClose={() => setBuyNowOpen(false)} product={{ ...productData, price: currentPrice }} quantity={qty} />
    </div>
  );
}